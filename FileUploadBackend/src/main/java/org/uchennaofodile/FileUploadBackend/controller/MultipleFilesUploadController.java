package org.uchennaofodile.FileUploadBackend.controller;
...
import org.uchennaofodile.FileUploadBackend.service.FileServiceImplementation;
import org.uchennaofodile.FileUploadBackend.util.FileUtil;
import java.io.File;
    ...
    public ResponseEntity<FileUploadResponse> uploadFiles(@RequestParam("files") MultipartFile[] files) {
        try {
            ...
            for (MultipartFile file : files) {
                ...
                MyFile fileModal = new MyFile(fileName, sourceFileContent, fileContentType);
                // Adding file into fileList
                fileList.add(fileModal);
            }
            // Saving all the list item into database
            fileServiceImplementation.saveAllFiles(fileList);
